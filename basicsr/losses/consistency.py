...
class IDMRFLoss(nn.Module):
    ...
    # https://github.com/shepnerd/inpainting_gmcnn/blob/master/pytorch/model/loss.py
    def __init__(self):
    ...
    def patch_extraction(self, featmaps):
        patch_size = 1
        patch_stride = 1
        # unfold：在featmaps进行滑动窗口操作，将featmaps切割成若干patch
    ...
    def _mrf_loss(self, o, t):
        ...
        # 计算所有o，t 所有 patch 之间的相似度
        for i in range(b_size):
            t_feat_i = t_normalized[i:i + 1, :, :, :]
            o_feat_i = o_normalized[i:i + 1, :, :, :]
            patches_OIHW = self.patch_extraction(t_feat_i)  # 将特征分成若干 patches

            # 输入特征为o_feat_i，卷积核为patches_OIHW --> 计算patch之间的相似度
