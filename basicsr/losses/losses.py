...
class Cosine_Similarity(nn.Module):
    def __init__(self, dim, eps=1e-8, loss_weight=0.5):
    ...
    def forward(self, x1, x2):
        return self.cos_sim(x1, x2) * self.loss_weight


@LOSS_REGISTRY.register()
class CLIP_CosSim_Img(nn.Module):
    '''
    计算两张图像的CLIP image feature之间的 余弦相似度
    '''
    def __init__(self, dim=0, eps=1e-8, loss_weight=0.5):
        super(CLIP_CosSim_Img, self).__init__()

        self.loss_weight = loss_weight
        self.cos_sim = nn.CosineSimilarity(dim=dim, eps=eps)

        if clip is None:
            raise ImportError('clip is required when CLIP_CosSim_Img is enabled.')
    ...
    def forward(self, x1, x2):
        ...
        x1_224 = F.interpolate(x1, size=(224,224))
        x2_224 = F.interpolate(x2, size=(224,224))
        img_feat1 = self.clip_model.encode_image(x1_224)
        img_feat2 = self.clip_model.encode_image(x2_224)

        sim = self.cos_sim(img_feat1, img_feat2) 
        sim = torch.mean(sim) * self.loss_weight
        # sim = torch.mean(F.cosine_similarity(img_feat1, img_feat2))
...
@LOSS_REGISTRY.register()
class Latent_ContrastLoss(nn.Module):
    def __init__(self, loss_weight=0.001, **kwargs):
        super(Latent_ContrastLoss, self).__init__()
        self.l1 = nn.L1Loss()
        self.loss_weight = loss_weight

    def forward(self, anchor, positive, negative):

        d_ap = self.l1(anchor, positive.detach())
        d_an = self.l1(anchor, negative.detach())
        contrastive = d_ap / (d_an + 1e-7)

        return self.loss_weight * contrastive

    # def L1_latent(self, a, p):
    #
...
class CrossEntropy(nn.Module):
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        ...
        return self.loss_weight * F.cross_entropy(pred.permute(0, 2, 1), target)
...
class GramLoss(nn.Module):
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        ...
        return self.loss_weight * self.criterion(self._gram_mat(pred), self._gram_mat(target), weight, reduction=self.reduction)
...
@LOSS_REGISTRY.register()
class GramFeat_ContrastLoss(nn.Module):
    """
    style loss
    """
    ...
    def __init__(self, 
        ...
        reduction='mean',
        criterion='l1', **kwargs):
        super(GramFeat_ContrastLoss, self).__init__()
        if reduction not in ['none', 'mean', 'sum']:
            raise ValueError(f'Unsupported reduction mode: {reduction}. Supported ones are: {_reduction_modes}')
    ...
    def forward(self, anchor, positive, negative,weight=None, **kwargs):
        """
        Args:
            pred (Tensor): of shape (N, C, H, W). Predicted tensor.
            target (Tensor): of shape (N, C, H, W). Ground truth tensor.
        ...
        d_ap = self.criterion(self._gram_mat(positive.detach()), self._gram_mat(anchor))
        d_an = self.criterion(self._gram_mat(negative.detach()), self._gram_mat(anchor))
