...
import numpy as np
from basicsr.archs.Transformer.Layers import EncoderLayer3,EncoderLayer4
...
class Encoder_patch77(nn.Module):
    ...
    def __init__(self, d_word_vec=516, n_layers=6, n_head=8, d_k=64, d_v=64,
        ...
        self.n_position = n_position
        self.dropout = nn.Dropout(p=dropout)
        self.cross = EncoderLayer4(d_model, d_inner, n_head, d_k, d_v, dropout=dropout)
        self.layer_stack = nn.ModuleList([
            EncoderLayer3(d_model, d_inner, n_head, d_k, d_v, dropout=dropout)
    ...
    def forward(self, src_fea, src_location,src_fea2, return_attns=False, src_mask=None):
        enc_output = src_fea
        enc_cross = src_fea2
        enc_output, enc_slf_attn = self.cross(enc_output,enc_cross,slf_attn_mask = src_mask)
        for enc_layer in self.layer_stack:
            enc_output, enc_slf_attn = enc_layer(enc_output, slf_attn_mask=src_mask)
        return enc_output
