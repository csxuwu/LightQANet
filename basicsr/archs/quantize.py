...
class VectorQuantizer_WeightCodebookLoss(nn.Module):
    ...
    def forward(self, z, gt_indices=None, z_gt=None, seg_feat=None, current_iter=None, weight_alpha=None, is_train=True):
        """
        Args:
            z: input features to be quantized, z (continuous) -> z_q (discrete)
               z.shape = (batch, channel, height, width)
        ...
        if z_gt is not None:
            kl_loss = F.kl_div(z.softmax(dim=-1).log(), z_gt.softma(dim=-1), reduction='sum')
            print(kl_loss.size())
