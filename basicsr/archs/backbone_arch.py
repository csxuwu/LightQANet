...
class Backbone_YOLOv7(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.stem(x)    # 320, 320, 64
        feat_stem = x
        # print(f'stem: {x.shape}')
        x = self.dark2(x)   # 160, 160, 256
        feat_dark2 = x
        # print(f'dark2: {x.shape}')
        ...
        feat2 = x
        #-----------------------------------------------#
        #   dark5的输出为 20, 20, 1024，是一个有效特征层
        #-----------------------------------------------#
        x = self.dark5(x)
        feat3 = x

        output = {}
        output['stem'] = feat_stem
        output['dark2'] = feat_dark2
        output['dark3'] = feat1
        output['dark4'] = feat2
        output['dark5'] = feat3
