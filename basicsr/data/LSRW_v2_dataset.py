...
class LSRW_v2_Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(LSRW_v2_Dataset, self).__init__()
        self.opt = opt
        ...
        if self.opt['phase'] == 'train':
            ...
            # 测试集：输入图像的描述
            self.test_input_captions_path = os.path.join(opt['dataroot'], 'low_captions.json')
            # print(self.test_input_captions_path)
            self.test_input_captions = json.load(open(self.test_input_captions_path))

            # 标签图像的描述
            self.test_target_captions_path = os.path.join(opt['dataroot'], 'tp.json')    # high_captions
