...
# 用于重构训练，input image = gt image
@DATASET_REGISTRY.register()
class All_LOL_Rec_Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(All_LOL_Rec_Dataset, self).__init__()
        self.opt = opt
        ...
        if self.opt['phase'] == 'train':
            ...
            self.imgs_ll = glob.glob(os.path.join(opt['dataroot'], 'low', '*.*'))
            self.imgs_gt = glob.glob(os.path.join(opt['dataroot'], 'high', '*.*'))

            if 'LOLv1' in opt['name']:
                self.imgs_ll = glob.glob(os.path.join(opt['dataroot'], 'our485', 'low', '*.*'))
                self.imgs_gt = glob.glob(os.path.join(opt['dataroot'], 'our485', 'high', '*.*')) 
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            imgs_ll_path = gt_path
            # gt_path = imgs_ll_path.replace('/low', '/high')
            ...
            # gt_path = imgs_ll_path.replace('lowlight', 'gt')
            ...
            img_refer_path = self.imgs_ll[index]        # 此时 reference image 不起作用
            img_refer = cv2.imread(img_refer_path).astype(np.float32) / 255.0

            # 计算每个通道的均值，注意OpenCV读取的格式是BGR而非RGB
            mean_b = torch.tensor(np.mean(img_gt[:, :, 0]))   # 蓝色通道均值
            mean_g = torch.tensor(np.mean(img_gt[:, :, 1]))  # 绿色通道均值
            mean_r = torch.tensor(np.mean(img_gt[:, :, 2]))   # 红色通道均值   
            light = torch.tensor(np.mean(img_gray))       # 灰度图均值
            global_contrast = torch.tensor(np.std(img_gray))  # 全局对比度
