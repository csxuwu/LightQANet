...
class LOLv1_v2_Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(LOLv1_v2_Dataset, self).__init__()
        self.opt = opt
        ...
        if self.opt['phase'] == 'train':
            ...
            self.imgs_ll = glob.glob(os.path.join(opt['dataroot'], 'eval15', 'low', '*.*'))
            self.imgs_gt = glob.glob(os.path.join(opt['dataroot'], 'eval15', 'high', '*.*'))
            self.img_size = opt.get('gt_size', 512)

            # 测试集：输入图像的描述
            self.test_input_captions_path = os.path.join(opt['dataroot'], 'eval15', 'low_captions.json')
            self.test_input_captions = json.load(open(self.test_input_captions_path))

            # 标签图像的描述
            self.test_target_captions_path = os.path.join(opt['dataroot'], 'eval15', 'high_captions.json')
