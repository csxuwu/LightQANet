...
class LOLv2_Real_v2_Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(LOLv2_Real_v2_Dataset, self).__init__()
        self.opt = opt
        ...
        if self.opt['phase'] == 'train':
            self.imgs_ll = glob.glob(os.path.join(opt['dataroot'], 'Train', 'Low', '*.*'))
            self.imgs_gt = glob.glob(os.path.join(opt['dataroot'], 'Train', 'Normal', '*.*'))
            ...
            self.imgs_ll = glob.glob(os.path.join(opt['dataroot'], 'Test', 'Low', '*.*'))
            self.imgs_gt = glob.glob(os.path.join(opt['dataroot'], 'Test', 'Normal', '*.*'))
            self.img_size = opt.get('gt_size', 512)

            # 测试集：输入图像的描述
            self.test_input_captions_path = os.path.join(opt['dataroot'], 'Test', 'Low_captions.json')
            self.test_input_captions = json.load(open(self.test_input_captions_path))

            # 标签图像的描述
            self.test_target_captions_path = os.path.join(opt['dataroot'], 'Test', 'Normal_captions.json')
    ...
    def __getitem__(self, index):
        ...
        # elif self.opt['phase'] == 'val':
            ...
            imgs_ll_path = self.imgs_ll[index]  # 低照度，返回下标为index的低照度图片路径
            gt_path = self.imgs_gt[index]
            # gt_path = imgs_ll_path.replace('/test', '/test_high')
