...
# -----------------------------------------
# 2024.05.20
# 基于LLIE_Prior_Prompt22_model

# 添加了 light pass fitler，用来衡量特征的照度，它是单独训练好的，此时不再进行训练。
# -----------------------------------------
...
class LightQANet_Model(BaseModel):
    def __init__(self, opt):
        ...
        self.total_index_counts = torch.zeros(1024, dtype=torch.int, device=self.device)
    ...
    def init_training_settings(self):
        ...
        self.cri_light_pass_filter = None
        ...
        if train_opt.get('light_pass_filter_opt'):

            # 定义 light pass filter
            self.LightPassFilter1 = LightPassFilter_conv1(8256).to(self.device) # 8256
            self.LightPassFilter2 = LightPassFilter_res1(32896).to(self.device)

            load_path1 = train_opt['light_pass_filter_opt']['load_path1']
            load_path2 = train_opt['light_pass_filter_opt']['load_path2']
            self.load_network(self.LightPassFilter1, load_path1, False)
            self.load_network(self.LightPassFilter2, load_path2, False)

        if train_opt.get('light_pass_filter2_opt'):

            # 定义 light pass filter
            self.LightPassFilter1 = LightPassFilter_conv1(8256).to(self.device) # 8256
            self.LightPassFilter2 = LightPassFilter_res1(32896).to(self.device)

            load_path1 = train_opt['light_pass_filter2_opt']['load_path1']
            load_path2 = train_opt['light_pass_filter2_opt']['load_path2']
            self.load_network(self.LightPassFilter1, load_path1, False)
            self.load_network(self.LightPassFilter2, load_path2, False)
    ...
    def optimize_parameters(self, current_iter):
        ...
        # --------------------------------------------------------------
        with torch.no_grad():
            gt_indices, quant_gt, feat_to_quant_gt, after_quant_feat_gt, gt_feat_hq_dict = self.net_hq.encode_indices(input=self.gt)
        ...
        # =======================================================================
        # 风格对比损失
        if self.cri_gram_feat_contrast:

            if train_opt['gram_feat_contrast_opt'].get('type_encoder', None):
                if train_opt['gram_feat_contrast_opt']['type_encoder'] == 'lq_encoder':
                    # 采用 lq encoder 来获得 特征
                    lq_encoder_out_gt = self.net_g.encoder(self.gt)
                    lq_encoder_out_rec = self.net_g.encoder(self.output)
                    lq_encoder_out_lq = self.net_g.encoder(self.lq)

                    l_content = self.cri_gram_feat_contrast(lq_encoder_out_rec['enc_feats_context'], lq_encoder_out_gt['enc_feats_context'], lq_encoder_out_lq['enc_feats_context'])

            else:
                with torch.no_grad():
                    _, quant_ouput, feat_to_quant_output, _ = self.net_hq.encode_indices(input=self.output)
                l_content = self.cri_gram_feat_contrast(feat_to_quant_output, feat_to_quant_gt, quant_g)
            l_content *= train_opt['gram_feat_contrast_opt']['loss_weight']
            l_g_total += l_content
            loss_dict['l_gram_feat_contrast'] = l_content

        # =======================================================================
        if self.cri_content:
            # 增强后的图像的特征分布 与 gt 的保持一致
            # 此处 计算特征间的差异。
            # 下一步还可以将特征二次处理，得到均值和方差，然后再进行分布差异的计算
            if train_opt['content_opt'].get('type_encoder', None):
                if train_opt['content_opt']['type_encoder'] == 'lq_encoder':
                    # 采用 lq encoder 来获得 特征
                    lq_encoder_out_gt = self.net_g.encoder(self.gt)
                    lq_encoder_out_rec = self.net_g.encoder(self.output)
                    l_content = self.cri_content(lq_encoder_out_rec['enc_feats_context'], lq_encoder_out_gt['enc_feats_context'])

            else:

                f_output = self.net_hq.multiscale_encoder(self.output)      
                f_gt = self.net_hq.multiscale_encoder(self.gt)      
                l_content = self.cri_content(f_output, f_gt)
        ...
        if train_opt.get('domain_Loss_opt'):
            ...
            if current_iter % it == 0:
                ...
                # domain_d_out_gt = self.net_domain_d(gt_feat)
                # fake_domain_label = torch.FloatTensor(domain_d_out_gt.data.size()).fill_(gt_domain).to(self.device)
                # loss_fake = criterion(domain_d_out_gt, fake_domain_label) / 2 * train_opt['domain_Loss_opt']['loss_weight']

                # l_g_total += loss_fake
                # loss_dict['l_domain_mse_ll_g'] = loss_fake 
        ...
        if train_opt.get('light_pass_filter_opt'):
            ...
            gt_feat0 = gt_feats_dict['128']
            gt_feat1 = gt_feats_dict['64']
            ...
            light_pass_filter_loss = light_pass_filter_loss * train_opt['light_pass_filter_opt']['loss_weight']
            l_g_total += light_pass_filter_loss
            loss_dict['l_light_pass_filter_loss'] =light_pass_filter_loss

        if train_opt.get('light_pass_filter2_opt'):
            # 拉近  增强图像、低照度图像、GT图像三者之间的距离
            lq_feat0 = self.out_dict['enc_feat_dict']['128']
            lq_feat1 = self.out_dict['enc_feat_dict']['64']
            lq_feats = {'layer0': lq_feat0, 'layer1':lq_feat1}
            ...
            out_feats_dict = self.net_g.encoder(self.output)
            out_feat0 = out_feats_dict['128']
            out_feat1 = out_feats_dict['64']
            out_feats = {'layer0': out_feat0, 'layer1':out_feat1}
            ...
            for idx, layer in enumerate(self.feat_weights):
                b_feature = lq_feats[layer]
                a_feature = gt_feats[layer]
                c_feature = out_feats[layer]

                na,da,ha,wa = a_feature.size()
                nb,db,hb,wb = b_feature.size()
                nc,dc,hc,wc = c_feature.size()
                ...
                for batch_idx in range(na):
                    b_gram = gram_matrix(b_feature[batch_idx])
                    a_gram = gram_matrix(a_feature[batch_idx])
                    c_gram = gram_matrix(c_feature[batch_idx])
                    ...
                    vector_b_gram = b_gram[torch.triu(torch.ones(b_gram.size()[0], b_gram.size()[1])).requires_grad_() == 1].requires_grad_()
                    vector_a_gram = a_gram[torch.triu(torch.ones(a_gram.size()[0], a_gram.size()[1])).requires_grad_() == 1].requires_grad_()
                    vector_c_gram = c_gram[torch.triu(torch.ones(c_gram.size()[0], c_gram.size()[1])).requires_grad_() == 1].requires_grad_()

                    light_factor_b = LightPassFilter(vector_b_gram)
                    light_factor_a = LightPassFilter(vector_a_gram)
                    light_factor_c = LightPassFilter(vector_c_gram)

                    half = int(light_factor_b.shape[0]/2)
                    
                    l_ab = self.feat_weights[layer]*torch.mean((light_factor_b/(hb*wb) - light_factor_a/(ha*wa))**2)/half/ b_feature.size(0)
                    l_ac = self.feat_weights[layer]*torch.mean((light_factor_c/(hb*wb) - light_factor_a/(ha*wa))**2)/half/ b_feature.size(0)
                    l_bc = self.feat_weights[layer]*torch.mean((light_factor_b/(hb*wb) - light_factor_c/(ha*wa))**2)/half/ b_feature.size(0)
                    layer_fsm_loss += l_ab + l_ac + l_bc
                loss_fsm += layer_fsm_loss / na

            light_pass_filter_loss = loss_fsm / (idx+1)
            light_pass_filter_loss = light_pass_filter_loss * train_opt['light_pass_filterw_opt']['loss_weight']
            l_g_total += light_pass_filter_loss
            loss_dict['l_light_pass_filter_loss'] =light_pass_filter_loss
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img, save_as_dir, visual_codebook=False):
        ...
        # 统计 codebook 激活的索引，需要手动设置存储路径
        # 查看每个索引的计数
        # plt.figure(figsize=(14, 7))
        # plt.bar(range(1024), self.total_index_counts.cpu(), color='blue')
        # plt.title('Histogram of Index Occurrences')
        # plt.xlabel('Index')
        # plt.ylabel('Frequency')
        # plt.grid(True)
        # name_pt = 'LOLv2_real_LL_baseline'
        # plt.savefig(f'/data/xuwu/codes/low_light/PromptEnhance/codebook_active_map/{name_pt}.png')  # Save the plot as a PNG file
        # torch.save(self.total_index_counts, f'/data/xuwu/codes/low_light/PromptEnhance/codebook_active_map/{name_pt}.pt')
